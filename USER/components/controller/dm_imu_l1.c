...
void IMU_RequestData(CAN_HandleTypeDef* hcan,uint16_t can_id,uint8_t reg)
{
    ...
    uint8_t motor_can_send_data[4]={(uint8_t)can_id,(uint8_t)(can_id>>8),reg,0xCC};
    motor_tx_message.StdId = 0x6FF;
    motor_tx_message.IDE = CAN_ID_STD;
    motor_tx_message.RTR = CAN_RTR_DATA;
    motor_tx_message.DLC = 4;
 ...
 * @brief 获取达妙陀螺仪的加速度命令
 */
void DM_imu_l1_get_accel(CAN_HandleTypeDef* hcan,uint16_t can_id)
{
    IMU_RequestData(hcan,can_id,DM_IMU_L1_ACCEL_REG);
}
/**
 * @brief 获取达妙陀螺仪的角速度命令
 */
void DM_imu_l1_get_gyro(CAN_HandleTypeDef* hcan,uint16_t can_id)
{
    IMU_RequestData(hcan,can_id,DM_IMU_L1_GYRO_REG);
}
/**
 * @brief 获取达妙陀螺仪的欧拉角命令
 */
void DM_imu_l1_get_angle(CAN_HandleTypeDef* hcan,uint16_t can_id)
{
    IMU_RequestData(hcan,can_id,DM_IMU_L1_ANGLE_REG);
}
/**
 * @brief 获取达妙陀螺仪的四元数命令
 */
void DM_imu_l1_get_quaternion(CAN_HandleTypeDef* hcan,uint16_t can_id)
{
    IMU_RequestData(hcan,can_id,DM_IMU_L1_QUAT_REG);
}
