...
gimbal_behaviour_e last_gimbal_behaviour = GIMBAL_ZERO_FORCE; //记录上次的行为模式
uint8_t center_flag = 0; //圆心标志
uint8_t radius_flag = 0;
uint8_t auto_first_cal = 0;
fp32 radius = 0.0f; //半径
fp32 angle = 0.0f; //角度
fp32 test1,test2;
uint8_t last_AT_key = 0; //上次自动模式切换按键
...
void gimbal_behaviour_mode_set(gimbal_control_t *gimbal_mode_set)
{
    ...
    else if(gimbal_behaviour == GIMBAL_TEST)
    {
        gimbal_mode_set->gimbal_yaw_motor.gimbal_motor_mode = GIMBAL_MOTOR_TEST;
        gimbal_mode_set->gimbal_pitch_motor.gimbal_motor_mode = GIMBAL_MOTOR_TEST;
    }
...
int gimbal_test_flag = 1;
void gimbal_isTest(gimbal_control_t *gimbal_isTest)
{
    if(gimbal_isTest->gimbal_rc_ctrl->rc.ch[4] >= 100)
		{
			gimbal_test_flag = 1;
		}
		else if(gimbal_isTest->gimbal_rc_ctrl->rc.ch[4] >= -100)
		{
			gimbal_test_flag = 0;
		}
}
...
static void gimbal_behavour_set(gimbal_control_t *gimbal_mode_set)
{
    ...
    else if (switch_is_mid(gimbal_mode_set->gimbal_rc_ctrl->rc.s[GIMBAL_MODE_CHANNEL]))
    {
        ...
        if( gimbal_mode_set->gimbal_rc_ctrl->mouse.press_r == 1) //鼠标右键开启自瞄
        {
            gimbal_behaviour = GIMBAL_AUTO;
        }
        else if (gimbal_mode_set->gimbal_rc_ctrl->mouse.press_r == 0)
        {
...
//自瞄模式控制
float test_pitch,yaw_test,pitch_test;
int fire_mode = 0;
int khf;
static void gimbal_autoangle_control(fp32 *yaw, fp32 *pitch, gimbal_control_t *gimbal_control_set)
{
...
static void gimbal_cali_control(fp32 *yaw, fp32 *pitch, gimbal_control_t *gimbal_control_set)
{
    ...
    if (gimbal_control_set->gimbal_cali.step == GIMBAL_CALI_PITCH_MAX_STEP)
    {
        ...
        *yaw = 0;
        ...
        gimbal_control_set->gimbal_pitch_motor.gimbal_motor_measure->motor_pitch_measure->ecd, gimbal_control_set->gimbal_cali.step);
    }
    ...
    if (gimbal_control_set->gimbal_cali.step == GIMBAL_CALI_YAW_MAX_STEP)
    {
        ...
        *yaw = GIMBAL_CALI_MOTOR_SET;
        ...
        gimbal_control_set->gimbal_yaw_motor.gimbal_motor_measure->motor_yaw_measure->ecd, gimbal_control_set->gimbal_cali.step);
    }
